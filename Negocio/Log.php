...
class Log
{
    ...
    {
        $this->conexion->abrir();
        $this->conexion->ejecutar($this-> logDAO ->cantidadPaginasFiltro($filtro));
        return $this->conexion->extraer();
    }
    ...
    {
        $this->conexion->abrir();
        $this->conexion->ejecutar($this-> logDAO ->listarFiltro($filtro, $cantidad, $pagina));
        $arrayLogs = array();
        while (($logActual = $this->conexion->extraer()) != null) {
            $newLog = new Log($logActual[0], $logActual[1], $logActual[2], $logActual[3], $logActual[4], $logActual[5], $logActual[6]);
            array_push($arrayLogs, $newLog);
        }
        $this->conexion->cerrar();
        return $arrayLogs;
    }
